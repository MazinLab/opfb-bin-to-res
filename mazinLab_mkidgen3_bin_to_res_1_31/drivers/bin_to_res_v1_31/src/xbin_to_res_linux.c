...
typedef struct {
    ...
    XBin_to_res_uio_map maps[ MAX_UIO_MAPS ];
} XBin_to_res_uio_info;

/***************** Variable Definitions **************************************/
static XBin_to_res_uio_info uio_info;
...
static int uio_info_read_name(XBin_to_res_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XBin_to_res_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XBin_to_res_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XBin_to_res_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XBin_to_res_Initialize(XBin_to_res *InstancePtr, const char* InstanceName) {
	XBin_to_res_uio_info *InfoPtr = &uio_info;
