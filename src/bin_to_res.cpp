...
#pragma HLS INTERFACE s_axilite port=rid_to_bin clock=S_AXI_clk name=resmap bundle=ctrl
#pragma HLS INTERFACE s_axilite port=group_align clock=S_AXI_clk name=align bundle=ctrl
...
#pragma HLS ARRAY_PARTITION variable=cache complete dim=1
	...
	replay_group++;
	cache_group = i_in.last ? cycleid_t(0):cycleid_t(cache_group+1);
}
