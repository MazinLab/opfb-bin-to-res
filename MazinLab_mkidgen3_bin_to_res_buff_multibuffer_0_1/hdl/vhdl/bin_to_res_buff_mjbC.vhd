...
entity bin_to_res_buff_mjbC is
    generic (
      ID : INTEGER := 1;            --core ID, unused in RTL
      din0_WIDTH : INTEGER := 512;  --data bitwidth
      din1_WIDTH : INTEGER := 512;    --shift control bitwidth
      dout_WIDTH : INTEGER := 512;  --output bitwidth
      OP : INTEGER := 1;            --opcode: 0-shl, 1-lshr, 2-ashr
      NUM_STAGE : INTEGER := 4      --stage number
  );
    port (
      clk : IN STD_LOGIC;
      reset : IN STD_LOGIC;
...
end entity bin_to_res_buff_mjbC;
architecture rtl of bin_to_res_buff_mjbC is
